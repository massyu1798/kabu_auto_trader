...
Responsibilities:
- Convert AM/PM/ONG Trade lists to a unified DataFrame (detail rows)
...
def build_trades_df(
    morning_trades: list,
    afternoon_trades: list,
    overnight_trades: list | None = None,
) -> pd.DataFrame:
    """
    Merge AM, PM and (optionally) ONG trades into a single DataFrame.
    Returns DataFrame sorted by entry_dt.
    """
    am_rows = trades_to_rows(morning_trades, "AM")
    pm_rows = trades_to_rows(afternoon_trades, "PM")
    ong_rows = trades_to_rows(overnight_trades or [], "ONG")
...
def build_daily_pnl(df: pd.DataFrame, last_days: int | None = None) -> pd.DataFrame:
    ...
    daily = pd.DataFrame({
        ...
        "TOTAL_cnt": (
            pivot_cnt["AM"] + pivot_cnt["PM"] + pivot_cnt["ONG"]
        ).astype(int).values,
    })
...
def get_latest_day_summary(df: pd.DataFrame) -> dict:
    ...
    Returns dict with keys: day, AM_pnl, PM_pnl, ONG_pnl, TOTAL,
                             AM_cnt, PM_cnt, ONG_cnt, TOTAL_cnt
    """
    if df.empty:
        return {}
...
def print_daily_table(daily_df: pd.DataFrame) -> None:
    ...
    if "ONG_pnl" in display.columns:
        display["ONG_pnl"] = display["ONG_pnl"].map(lambda x: f"{x:+,.0f}")
    display["TOTAL"] = display["TOTAL"].map(lambda x: f"{x:+,.0f}")

    ong_in_data = "ONG_pnl" in daily_df.columns

    if HAS_TABULATE:
        if ong_in_data:
            rows = display[
                ["day", "AM_pnl", "AM_cnt", "PM_pnl", "PM_cnt",
                 "ONG_pnl", "ONG_cnt", "TOTAL", "TOTAL_cnt"]
            ].values.tolist()
            headers = ["Day", "AM PnL", "AM#", "PM PnL", "PM#",
                       "ONG PnL", "ONG#", "TOTAL", "#"]
        else:
            rows = display[
                ["day", "AM_pnl", "AM_cnt", "PM_pnl", "PM_cnt", "TOTAL", "TOTAL_cnt"]
            ].values.tolist()
        ...
        if ong_in_data:
            ...
            header = (f"  {'Day':<12} {'AM PnL':>12} {'AM#':>4}"
                      f" {'PM PnL':>12} {'PM#':>4} {'TOTAL':>12} {'#':>4}")
        print(header)
        print("  " + "-" * (len(header) - 2))
        for _, row in display.iterrows():
            if ong_in_data:
                print(
                    f"  {row['day']:<12} {row['AM_pnl']:>12} {row['AM_cnt']:>4} "
                    f"{row['PM_pnl']:>12} {row['PM_cnt']:>4} "
                    f"{row['ONG_pnl']:>12} {row['ONG_cnt']:>5} "
...
def print_latest_day(summary: dict) -> None:
    ...
    print(f"  [ONG] PnL: {summary.get('ONG_pnl', 0):>+14,.0f} 円  ({summary.get('ONG_cnt', 0)} trades)")
