...
class OvernightGapEngine:
    def __init__(self, config_path: str = "config/strategy_config.yaml"):
        ...
        self.commission_rate = g.get("commission_rate", 0.0)
        self.slippage_rate = g.get("slippage_rate", 0.001)
        self.max_daily_loss_pct = g.get("max_daily_loss", 0.025)
        self.weekly_max_loss_pct = g.get("weekly_max_loss", 0.05)
        self.monthly_max_loss_pct = g.get("monthly_max_loss", 0.10)
        self.max_positions = g.get("max_positions", 3)
        ...
        self.score_threshold = ong_params.get("signal_score_threshold", 1.1)
        self.risk_per_trade = g.get("risk_per_trade", 0.01)
        self.lot_unit = 100
    ...
    def _night_gap_proxy_ok(self, date) -> bool:
        ...
        if today_close <= 0:
            return True
        return (next_open / today_close - 1.0) >= _NIGHT_GAP_PROXY_MIN
